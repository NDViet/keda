...
- **Selenium Grid Scaler**: Add `jobScalingStrategy` trigger metadata (`default`, `custom`, `accurate`, `eager`) so the queue metric matches the ScaledJob scaling strategy, avoiding runaway Node creation from double-counting on-going sessions under `accurate`/`eager` ([#3167](https://github.com/SeleniumHQ/docker-selenium/issues/3167))
