...
- **Selenium Grid Scaler**: Add `jobScalingStrategy` trigger metadata so the queue metric is compatible with the ScaledJob `accurate` strategy, avoiding runaway Node creation from double-counting on-going sessions ([#3167](https://github.com/SeleniumHQ/docker-selenium/issues/3167))
